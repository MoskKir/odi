...
const SECTORS: Sector[] = [
  {
    core: 'грусть',
    color: '#4878B8', colorMid: '#6898D0', colorLight: '#98B8E0',
    level1: ['одиночество', 'подавленность', 'вина', 'стыд', 'скука', 'вялость'],
    level2: ['робость', 'глупость', 'несчастье', 'неадекватность', 'неполноценность', 'безразличие'],
  },
  {
    core: 'бешенство',
    color: '#D07030', colorMid: '#E09050', colorLight: '#F0B888',
    level1: ['враждебность', 'злость', 'свирепость', 'ненависть', 'придирчивость'],
    level2: ['зависть', 'эгоизм', 'разочарование', 'ярость', 'раздражение', 'скептичность'],
  },
  {
    core: 'страх',
    color: '#906048', colorMid: '#B08868', colorLight: '#D0B098',
    level1: ['покинутость', 'смущение', 'беспомощность', 'покорность', 'безнадежность', 'тревожность'],
    level2: ['озадаченность', 'обескураженность', 'ничтожность', 'слабость', 'неловкость', 'нелепость'],
  },
  {
    core: 'радость',
    color: '#D08080', colorMid: '#E8A8A0', colorLight: '#F8C8C0',
    level1: ['восторг', 'чувственность', 'энергичность', 'игривость', 'творчество', 'осознание'],
    level2: ['смелость', 'очарование', 'возбуждение', 'удовольствие', 'экстравагантность', 'наслаждение', 'бодрость'],
  },
  {
    core: 'сила',
    color: '#C0A020', colorMid: '#D8C040', colorLight: '#E8D880',
    level1: ['гордость', 'уважение', 'признание', 'уверенность', 'великодушие', 'преданность', 'сдержанность'],
    level2: ['удовлетворённость', 'авторитетность', 'значимость', 'разумность', 'решительность', 'благодарность'],
  },
  {
    core: 'спокойствие',
    color: '#509840', colorMid: '#78B868', colorLight: '#A0D090',
    level1: ['умиротворённость', 'задумчивость', 'близость', 'нежность', 'доверие'],
    level2: ['созерцательность', 'расслабленность', 'отзывчивость', 'безмятежность', 'чувствительность'],
  },
]
...
export function EmotionWheel({ isOpen, onClose }: Props) {
  ...
  const handleSelect = (emotion: string) => {
    ...
    dispatch(setEmotion(next as any))
    if (sessionId && next) {
  ...
  const segments = useMemo(() => {
    ...
    SECTORS.forEach((sector, si) => {
      ...
      const coreFontSize = fitFontSize(sector.core, R0, R1, cAngle, 'tangential', 3.0)
      result.push({
        path: arcPath(R0, R1, cA1, cA2),
        fill: sector.color, label: sector.core,
        x: cx, y: cy,
        rotate: tangentialRotation(coreMid),
        fontSize: coreFontSize, fontWeight: 800, ring: 0, textColor: '#fff',
      })
      ...
      sector.level1.forEach((lbl, i) => {
        ...
        result.push({
          ...
          fill: sector.colorMid, label: lbl,
          x, y, rotate: radialRotation(mid),
          fontSize: fs, fontWeight: 600, ring: 1, textColor: '#222',
        })
      })
      ...
      sector.level2.forEach((lbl, i) => {
        ...
        result.push({
          ...
          fill: sector.colorLight, label: lbl,
          x, y, rotate: radialRotation(mid),
          fontSize: fs, fontWeight: 400, ring: 2, textColor: '#333',
        })
      })
    })
      ...
      <div className="flex flex-col items-center p-3">
        <div className="flex items-center justify-between w-full mb-1 px-1">
          ...
          <div className="flex items-center gap-2">
            {currentEmotion && (
              <span className="text-xs text-odi-accent font-medium px-2 py-0.5 rounded-full bg-odi-accent/10">
                {currentEmotion}
              </span>
            )}
            {currentEmotion && (
              <Button minimal small icon="cross" title="Сбросить"
                onClick={() => { dispatch(setEmotion(null)); onClose() }}
                className="!text-odi-text-muted"
              />
            )}
            <Button minimal small icon="small-cross" onClick={onClose} className="!text-odi-text-muted" />
          </div>
        </div>

        <div className="h-6 flex items-center justify-center">
          {hovered && <span className="text-sm text-odi-text font-semibold animate-fade-in">{hovered}</span>}
        </div>
        ...
        >
          {segments.map((seg, i) => {
            const isActive = currentEmotion === seg.label
            const isHov = hovered === seg.label
            return (
              <g key={i}>
                <path
                  d={seg.path}
                  fill={isActive ? '#4A7FC0' : seg.fill}
                  stroke={isActive ? '#3060A0' : '#fff'}
                ...
                <text
                  ...
                  fill={isActive ? '#fff' : seg.textColor}
